...
var CountriesFilter = React.createClass({
    mixins: [
        Reflux.connect(CountryStore, 'countries')
    ],
    propTypes: {
    ...
    renderCountries: function () {
        ...
        return (
            <div className="list-group">
                <a className="list-group-item" style={{padding:"8px 8px"}} key={'全部'}
                   onClick={self.handleItemClick}>
                    全部
                </a>
                { this.state.countries.map(function (item) {
                    return (
                        <a className="list-group-item" style={{padding:"8px 8px"}} key={item.term} data-name={item.term}
                           onClick={self.handleItemClick}>
    ...
    render: function () {
        ...
        var buttonTitle = this.state.show ? this.props.buttonHideTitle : this.props.buttonShowTitle;
        var classes = classNames('filter-box',
            {'in': this.state.show});
        return (<span>
        <button onClick={this.handleClick} className="btn btn-default btn-sm">
            <span className="fa fa-bars">
            </span> {buttonTitle}</button>
        <div className={classes}>
            <div className='filter-box-header'>
                <h1>{this.props.title}<span className="fa fa-times" aria-hidden="true" onClick={this.handleClick}
      ...
      <div className='list-wrap'>
        ...
        <div className='has-top-bar'>
          ...
          <ArtistList
            ...
            loaded={this.state.artists.loaded}
